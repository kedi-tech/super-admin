...
import { deactivateDeviceAction } from "@/actions/licenses"
import { useTransition } from "react"

export function LicenseDevicesTab({ devices, licenseId }: { devices: Device[]; licenseId: string }) {
  const [isPending, startTransition] = useTransition()

  if (devices.length === 0) {
    return <p className="text-sm text-zinc-400 py-4">No devices activated.</p>
  }
    ...
    <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
      <table className="w-full text-sm">
        <thead className="bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800">
          <tr>
            {["Machine", "OS", "IP", "Status", "Activated", "Last Seen", ""].map((h) => (
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
          {devices.map((d) => (
            <tr key={d.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/50">
              ...
              <td className="px-4 py-3 font-mono text-xs text-zinc-400">{d.ipAddress ?? "—"}</td>
              ...
              <td className="px-4 py-3">
                {d.status === "ACTIVE" && (
                  <Button
                    ...
                    disabled={isPending}
                    onClick={() => startTransition(() => deactivateDeviceAction(d.id, licenseId))}
                  >
                    Deactivate
                  </Button>
                )}
